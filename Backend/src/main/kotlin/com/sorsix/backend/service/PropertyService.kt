...
import com.sorsix.backend.api.dtos.PropertyAvailabilityDTO
...
import com.sorsix.backend.repository.guests.BookingGuestsRepository
...
class PropertyService(
    ...
    fun findAllProperties(
        ...
        return propertyRepository
            .findAllByFilterString(filterString, checkIn, checkOut, adults, children, pet)
            .map { dtoMapperService.mapPropertyToPropertyCardDTO(it) }
    }
...
//        this.propertyRepository.suggestProperties().map { this.mapPropertyToPropertyCardDTO(it) }
    ...
    fun reserveProperty(
        ...
        if (offerRequest.checkInDate.isAfter(
                offerRequest.checkOutDate,
            )
        ) {
            throw UnauthorizedAccessException("Check in date must be before check out date.")
        }
        ...
        if (offerRequest.numberOfAdults > property.guests) {
            throw PropertyCapacityException(
                "Number of adults exceeds the maximum number of guests allowed.",
            )
        }
        if (offerRequest.numberOfChildren > property.guests) {
            throw PropertyCapacityException(
                "Number of children exceeds the maximum number of guests allowed.",
            )
        }
        if (offerRequest.numberOfPets > property.guests) {
            throw PropertyCapacityException(
                "Number of pets exceeds the maximum number of guests allowed.",
            )
        }

        val bookingStatus =
            bookingStatusRepository.findById(BookingStatusEnum.WAITING_FOR_APPROVAL.ordinal.toLong())
    ...
    private fun getPropertyAvailability(
        ...
        this.propertyAvailabilityRepository
            ...
            ?: throw PropertyNotAvailableException(
                "Property is not available for the given period: $checkInDate - $checkOutDate",
            )
    ...
    fun getAllProperties(
        ...
        if (checkIn == null || checkOut == null) {
            if (decoded.isNotBlank()) {
                return this.propertyRepository
                    .findAllPaginatedByFilterStringWithoutDates(decoded, adults, children, pets, pageable)
                    .let { pages ->
                        PropertyResponse(
                ...
                return this.propertyRepository.findAllPaginated(pageable).let { pages ->
                    PropertyResponse(
                        content = pages.toList().map { dtoMapperService.mapPropertyToPropertyCardDTO(it) },
                        totalPages = pages.totalPages,
                        totalElements = pages.totalElements,
        ...
        val pages =
            this.propertyRepository
                .filterWithPagination(decoded, checkIn, checkOut, adults, children, pets, pageable)
