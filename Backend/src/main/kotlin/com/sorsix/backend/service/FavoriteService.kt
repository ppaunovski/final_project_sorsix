...
import com.sorsix.backend.repository.favorite.FavoriteRepository
...
class FavoriteService(
    ...
    fun findAllFavoritesForUser(authenticate: Authentication?): List<PropertyCardDTO> {
        ...
        val favorites =
            user?.let { this.favoriteRepository.findByUserId(it) }
                ?: throw UserAccountNotFoundException("User not found")
        val propertyList: List<PropertyCardDTO> =
    ...
    fun deleteFavoriteByUserIdAndPropertyId(
        authenticate: Authentication?,
        propertyId: Long,
    ): Long {
        if (authenticate == null) throw UnauthorizedAccessException("User not authenticated")
        val user =
            userAccountRepository.findByEmail(authenticate.name)?.id
                ?: throw UserAccountNotFoundException("User not found")
    ...
    fun saveFavorite(
        ...
        if (authenticate == null) throw UnauthorizedAccessException("User not authenticated")
        val user =
            userAccountRepository.findByEmail(authenticate.name)
                ?: throw UserAccountNotFoundException("User not found")
