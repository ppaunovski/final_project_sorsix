...
class UserAccountService(
    ...
    fun findAllUserAccounts() = userAccountRepository.findAll().map { dtoMapperService.mapUserAccountToDTO(it) }
    ...
    fun findUserByEmail(email: String) =
        userAccountRepository.findByEmail(email) ?: throw UserAccountNotFoundException(
    ...
    fun getProfitsPerProperty(auth: Authentication?): List<ProfitsPerPropertyDTO> {
        ...
        val user =
            userAccountRepository.findByEmail(auth.name)
                ?: throw UserAccountNotFoundException("User account with email ${auth.name} not found")
